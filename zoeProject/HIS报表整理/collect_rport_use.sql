...
connect by prior trim(a.class_code) = trim(a.super_class);
...
where trim(a.class_code) = trim(b.class_code); 
...
from COMM.DATA_OPERATE_LOG a, 
     REPORT.qr_report b
where a.operation_time > sysdate -90 
  and a.operate_data1 = b.report_id
...
with report_use as 
    (select operate_data1 as report_id,max(operation_time) as operation_time
     from COMM.DATA_OPERATE_LOG a
     where operation_time > sysdate - 365
