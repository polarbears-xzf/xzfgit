...
-如果插入的值中含有&，要做特殊处理，转换为"'||'&'||'",否则sql语句会把&后面的串当作变量变量处理
-*/
-
...
BEGIN
    ...
    ELSE
     ...
     lv_condition := lv_condition || ' OR CREATED_TIME >= to_date('''||to_char(id_last_date,'yyyy-mm-dd')||''',''yyyy-mm-dd''))';
    END IF;
  ...
  LOOP
    IF  lc_record.DATA_TYPE = 'VARCHAR2' THEN        
      lv_sql :=  lv_sql||'||'',''||'||'DECODE('||lc_record.COLUMN_NAME||',NULL,''NULL'',''''''''||REPLACE(' ||lc_record.COLUMN_NAME||',''&'',''''||''&''||'''')||'''''''')';
...
begin
    ...
    v_return := zoetmp.GET_MENU_FOR_INSERT('ZOECOMM','COM_MENU_TABLE','ZOETMP','COM_MENU_TABLE_TMP',TO_DATE('2019-07-24','YYYY-MM-DD'));
    v_return := zoetmp.GET_MENU_FOR_INSERT('ZOECOMM','COM_MENU_CONFIG','ZOETMP','COM_MENU_CONFIG_TMP',TO_DATE('2019-07-24','YYYY-MM-DD'));
