...
--  初始化META_OBJ$，从DBA_TABLES。排除Oracle用户，依据ZOEDEVOPS.ZOEPKG_UTILITY.GET_ORACLE_USER
    ...
    lv_sql := lv_sql || ' CREATED,SYS_CONTEXT(''USERENV'',''SESSION_USER'') AS MODIFIER,TO_DATE(TIMESTAMP,''YYYY-MM-DD HH24:MI:SS'')';
...
--  同步用户
  ...
  BEGIN
    ...
    SELECT SYS_GUID(),USERNAME,
            ...
            WHEN USERNAME = (SELECT COLUMN_VALUE FROM TABLE(ZOEDEVOPS.ZOEPKG_UTILITY.GET_ORACLE_USER) B WHERE B.COLUMN_VALUE=A.USERNAME) THEN 'ORACLE'
            ELSE '' END,
 ...
 END increment_sync_user;
  ...
  BEGIN
    increment_sync_user;
    increment_sync_object;
    increment_sync_table;
    increment_sync_column;
  EXCEPTION
    WHEN OTHERS THEN
        ROLLBACK;
        RAISE;
