...
BEGIN
	...
	FOR c1 IN (SELECT A.CON_DBID,A.DBID , A.INSTANCE_NUMBER, LEAD(SNAP_ID) OVER (ORDER BY A.DBID,A.SNAP_ID) NEXT_SNAP_ID,
		  SNAP_ID SNAP_ID, LEAD(SYS_TIME_VALUE) OVER (ORDER BY A.DBID,A.SNAP_ID) - SYS_TIME_VALUE SYS_TIME_VALUE
		FROM
		  (SELECT A.CON_DBID, A.DBID, A.INSTANCE_NUMBER, A.SNAP_ID,
